...
def search(request):
    ...
    cats = {item['category'] for item in catprods}
    for cat in cats:
        prod = Products.objects.filter(product_name__icontains=query)
        n = len(prod)
        nSlides = n // 4 + ceil((n / 4) + (n // 4))
        allProds.append([prod, range(1, nSlides), nSlides])
    params = {'allProds':allProds}
    return render(request, 'mshop/search.html',params)
...
def checkout(request):
    if request.method == "POST":
        ...
        update.save()
        mcorder = MilmaCurrentOrder(order_id = order.order_id, orderstatus = "order_placed", name=order.name, curlocation=order.curlocation, items_json=order.items_json)
        mcorder.save()
        thank = True
        id = order.order_id
